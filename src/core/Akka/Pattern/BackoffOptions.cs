...
namespace Akka.Pattern
{
    ...
    internal class BackoffOptionsImpl : BackoffOptions
    {
        ...
        public IBackoffReset BackoffReset => _reset ?? new AutoReset(_minBackoff);
        ...
        internal override Props Props
        {
            get
            {
                ...
                var autoReset = BackoffReset as AutoReset;
                if (autoReset != null)
                {
                ...
                {
                    return Props.Create(
                         () => new BackoffOnRestartSupervisor(_childProps, _childName, _minBackoff, _maxBackoff, BackoffReset, _randomFactor, _strategy));
                }
                else if (_backoffType is StopImpliesFailure)
                {
                    return Props.Create(
                        () => new BackoffSupervisor(_childProps, _childName, _minBackoff, _maxBackoff, BackoffReset, _randomFactor, _strategy));
                }
                else
                {
                    return Props.Empty;
